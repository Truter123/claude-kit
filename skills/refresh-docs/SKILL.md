---
name: refresh-docs
description: Regenerate a project's docs/domain/*.md (overview, business-rules, flows) from the current code, preserving answered open questions. Use when the domain docs have drifted, after a large feature lands, or to seed them on a project that has none. Argument = one doc name (overview | business-rules | flows) to refresh just that one, or nothing for all three.
---
...
and their statements in business language; bounded-context boundaries and responsibilities;
...
| `business-rules.md` | 150 lines | BR catalogue: id, the rule in business language, why it exists, and the class that enforces it as a lookup key — no file:line, no call chains |
